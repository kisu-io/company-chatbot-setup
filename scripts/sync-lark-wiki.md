---
source_url: https://open.feishu.cn/document/home
ingested: 2026-05-03
sha256: pending-compute-on-first-sync
---

# Lark/Feishu Wiki Sync Script

**Purpose:** Periodically sync Lark Wiki docs to local markdown files for Hermes Agent to query.
...
**Schedule:** Run every 6 hours via cron (optional)
...
cp ~/projects/company-chatbot/.env.example ~/.hermes/.env
# Edit ~/.hermes/.env with your Lark credentials
```
...
Save this as `~/.hermes/scripts/sync-lark-wiki.py`:
...
LARK_WIKI_URL = "https://open.feishu.cn/open-apis/wiki/v1/nodes"
...
# Check output
ls -la ~/projects/company-chatbot/company-wiki/raw/lark/
...
0 */6 * * * python ~/.hermes/scripts/sync-lark-wiki.py >> ~/.hermes/logs/lark-sync.log 2>&1
```
...
hermes cron create \
  --schedule "0 */6 * * *" \
  --prompt "Run Lark Wiki sync: python ~/.hermes/scripts/sync-lark-wiki.py" \
  --name "lark-wiki-sync" \
  --deliver local
```
...
| Error | Solution |
|-------|----------|
| `Auth failed` | Check app_id/app_secret in .env |
| `Permission denied` | Enable Docs:Read in Lark app permissions |
| `Rate limited` | Add `time.sleep(2)` between API calls |
| `Empty content` | Some docs may be restricted — check permissions |
| `Encoding error` | Ensure UTF-8 encoding in script |

---

## Next Steps After Sync

1. **Review synced docs:**
   ```bash
   cat ~/projects/company-chatbot/company-wiki/raw/lark/*.md | head -100
   ```

2. **Test Hermes queries:**
   ```
   "What does our wiki say about onboarding?"
   "Find all documents about engineering processes"
   ```

3. **Create concept pages** (manual or automated):
   - Extract key concepts from raw docs
   - Create `company-wiki/concepts/*.md` with cross-references

4. **Set up Q&A patterns:**
   - Common questions → pre-computed answers in `queries/`
   - Learning paths → curated sequences in `learning-paths/`
